"""Analyzer subgraph for JobScout v5.
...
v5 difference from v4: the node body is wrapped in a `v5.analyzer` span so
Phoenix shows the analyzer as a distinct child of the orchestrator span.
"""
import os
...
from v5.schemas import Plan, SearcherResult, FinalAnswer
...
def build_analyzer_graph():
    ...
    def analyzer_node(state: AnalyzerState):
        with tracer.start_as_current_span("v5.analyzer"):
            plan = state["plan"]
